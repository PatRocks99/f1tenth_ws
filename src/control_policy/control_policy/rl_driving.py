...
class RL_Drive(Node):
    ...
    def __init__(self):
        ...
        self.odom = self.create_subscription(Odometry(), '/odom', self.odom_callback, 1) #! check the topic name
    ...
    def lidar_callback(self, data):
        ...
        gap = ranges.length()/20
        if ranges.length() % 20 ==0:
            i = 0
            while i < 20:
                self.laser_observation.pop(0)
                self.laser_observation.append(ranges(i*gap))
                i+=1
            else:
                while ranges.length() % 20 != 0:
    ...
    def pub_drive(self, speed, steering_angle):
        ...
        drive_msg.header.stamp = self.get_clock().now().to_msg() #required for a stamped message
