...
class FollowTheGap(Node):
    ...
    def __init__(self):
        ...
        self.drive_pub=self.create_publisher(AckermannDriveStamped, '/drive', 1)  #! Trying somthing new(AckermannDriveStamped(), 'follow_the_gap', 1)
        #self.drive_pub=self.create_publisher(AckermannDriveStamped, '/follow_the_gap', 1)
        self.scan = self.create_subscription(LaserScan(), '/scan', self.lidar_callback, 1)
        self.scan  # prevents unused variable warning
        self.lidar_fov = np.radians(180)
        self.max_speed = 5

        self.pub_drive(0.0,0.0) #added so the steering angle and speed always reset to 0 before driving
    ...
    def find_max_gap(self, free_space_ranges):
        ...
        for i in range(len(free_space_ranges)):
            if free_space_ranges[i] > min(max(1.5, maxima/3), 4):  #! Make this adaptive at some point 1/2 the max length perhaps ## this is a bad idea as it may drive into wall this number becomes to small so use max
    ...
    def lidar_callback(self, data):
        ...
        ranges = self.preprocess_lidar(ranges) #! add this back later self.preprocess_lidar(data.ranges) 
        if ranges == 1:
        ...
        steering_angle = ((best_point)*data.angle_increment + data.angle_min) #((best_point+cut_points)*data.angle_increment + data.angle_min)#*(2.5-speed)*0.5
        ...
        #     if front_avg < 0.2:
        #         steering_angle = steering_angle/8
        speed = 1.25+(1/(steering_angle**2+0.01))/50 #1.25+(1/(steering_angle**2+0.01))/50 #this works but want to go faster #function adds max 2  #min(1.2 + ranges[int(len(ranges)/2)]/6, 3.5) #!changing speed to be based off of the steering angle
        self.max_speed = 1.25+(1/(0**2+0.01))/50 # make sure same as above
        self.pub_drive(speed,steering_angle)#publish drive message every time the lidar is published
    ...
    def pub_drive(self, speed, steering_angle):
        ...
        drive_msg.header.stamp = self.get_clock().now().to_msg() #Comented out because ackerman drive does not have a header. add back if stamped message is used
